...
- **Do:**
  ...
  - Docs: atualizado `docs/canonicos/UX_BLUEPRINT.md` com cards Dashboard: Entradas, Saídas, Já Pago, Falta Pagar; barra de resumo em Lançamentos (posição e fórmulas Total Geral/Total Pago).
  - Frontend:
    - `client/src/pages/Dashboard.tsx`: cards Saldo Real/Saldo Projetado substituídos por Já Pago/Falta Pagar.
    - `client/src/pages/Transactions.tsx`: barra de totais adicionada abaixo dos chips, baseada na lista filtrada visível.
- **Check:**
  - Build OK: `npm run build`.
  - Validação visual Playwright em `/` e `/transactions` confirmou novos cards e nova barra.
  - Limitação ambiente local: API com `ECONNREFUSED ::1:5433`, impede validação com massa real de dados.
- **Act:**
  - Registro de testes atualizado em `docs/logs/TEST_LOG.md`.
  - Próximo passo: validar em ambiente com banco ativo para confirmar recálculo com dados reais e finalizar issues no Linear.

## 2026-03-15 — Deploy producao (Coolify Hostinger) (DEV-166)
- **Plan:** Disparar deploy em producao via Coolify respeitando guardrail de dados (sem comandos de escrita de banco).
- **Do:** Deploy acionado no Coolify para `proj-financa-v1` (build pack Dockerfile) sem executar `db:push`/`db:seed`/backfill.
- **Check:** Smoke HTTP/HTTPS do dominio + validacao `/api` retornando `404` imediato (sem timeout).
...
- **Do:** (em andamento)
  - Docs: `docs/canonicos/UX_BLUEPRINT.md` atualizado para Layout 3.0, nova tela de Categorias e ajustes de Metodos de Pagamento/Lancamentos.
...
- **Plan:** Escopo definido para adicionar operacoes editar/excluir na Reserva de Emergencia, seguindo padrao existente em Goals e Investments.
- **Do:**
  ...
  - Frontend: modal de edicao, botoes editar/excluir, confirmacao de exclusao em `client/src/pages/Investments.tsx`.
...
- **Act:** Documentacao atualizada em `docs/canonicos/MODELO_DADOS.md`, `docs/canonicos/API_CONTRACT.md` e `docs/USAGE.md`.
...
- **Do:**
  ...
  - Criado `docs/logs/TEST_LOG.md` para historico de testes.
  - Criado `docs/logs/PDCA_LOG.md` para historico PDCA.
...
- **Plan:** Diagnosticar porque Node/Express nao ficava acessivel fora do localhost e padronizar deploy (nginx + systemd + firewall).
- **Do:**
...
- **Act:** Runbook atualizado em `docs/canonicos/RUNBOOK.md` com checklist de deploy (nginx/systemd/firewall/ingress OCI).

## 2026-02-14 — Migracao de deploy para Coolify (Hostinger VPS)
- **Plan:** Migrar fluxo de deploy manual em VM Oracle para deploy automatico no Coolify em nova VPS (`31.97.240.105`) e ajustar documentacao operacional.
- **Do:**
  - Configurado app no Coolify via MCP (`build_pack=dockerfile`, app `proj-financa-v1`).
  - Criado PostgreSQL no Coolify para projeto (`db-proj-financa-v1`) e obtido `internal_db_url`.
  ...
  - Atualizados `docs/canonicos/RUNBOOK.md`, `docs/USAGE.md` e `replit.md` para novo fluxo (Coolify + DNS Hostinger manual).
...
- **Act:** Publicar novo commit no GitHub e redeployar app no Coolify com `DATABASE_URL` apontando para Postgres criado.
...
- **Do:**
  - Docs: atualizado `docs/canonicos/API_CONTRACT.md` com POST/PATCH/DELETE `/api/categories` e erros `400/404/409`.
  - Docs: atualizado `docs/canonicos/UX_BLUEPRINT.md` com formulario inline de categoria e confirmacao inline de exclusao (sem modal).
  - Backend: validacoes para categorias (name/kind/monthlyBudgetCents) e bloqueio de exclusao com `409` quando categoria em uso por `transactions` ou `recurrences` (`server/storage.ts` + `server/index.ts`).
  - Frontend: UI em `client/src/pages/Transactions.tsx` com botao "+ Nova categoria", edicao e exclusao inline; recarrega lista e seleciona categoria criada no filtro.
- **Check:** Fluxo completo testado manualmente (criar/editar/excluir + erro de exclusao por uso).
- **Act:** Registro de testes atualizado em `docs/logs/TEST_LOG.md`.

## 2026-02-06 — Refinamento de UI (planilha mensal viva)
- **Plan:** Melhorar legibilidade e consistencia visual do front, sem alterar comportamentos fora do `docs/canonicos/UX_BLUEPRINT.md` (desktop-first, sem animacoes).
- **Do:**
...
- **Plan:** Simplificar navegacao do front-end reduzindo carga cognitiva e agrupando telas em menu lateral (desktop-first, sem animacoes), mantendo rotas e comportamentos existentes.
- **Do:**
  - Docs: adicionado layout de navegacao global e mapa de grupos em `docs/canonicos/UX_BLUEPRINT.md`.
  - Docs: alinhado design system de UI com novo menu lateral em `.interface-design/system.md`.
...
- **Check:** `npm run build` OK; output em chunks por pagina.
- **Act:** Registro de testes atualizado em `docs/logs/TEST_LOG.md`.

## 2026-03-13 — DEV-154: Erro de API em dev (mensagem vazia)
- **Plan:** Corrigir erro recorrente em dev onde API retornava `500` com mensagem genérica (sem indicar causa), bloqueando validação via Playwright.
- **Do:**
...
- **Check:**
  - `GET /api/categories` quando DB indisponível retorna erro explícito (ex: `connect ECONNREFUSED ::1:5433`), evitando "Internal Server Error" sem causa.
  - CRUDs prioritários validados via Playwright (evidências em `artifacts/playwright/dev-154-20260313/`).
- **Act:**
  - Registro de testes atualizado em `docs/logs/TEST_LOG.md`.
  - Comentário adicionado no Linear (DEV-154) com diagnóstico e evidências.
...
- **Do:**
  - Docs: atualizados `docs/canonicos/MODELO_DADOS.md`, `docs/canonicos/API_CONTRACT.md`, `docs/canonicos/UX_BLUEPRINT.md`.
  ...
  - Frontend: nova coluna "Pago" em `client/src/pages/Transactions.tsx` com checkbox apenas para `exit`, update otimista e feedback visual discreto.
...
- **Act:**
  - Registro de testes atualizado em `docs/logs/TEST_LOG.md` com evidencias da validacao.
...
- **Plan:** Publicar alteracao pendente no repositorio e garantir ambiente de producao no Coolify alinhado ao mesmo commit do `main`.
- **Do:**
  - Commit e push executados no `main` com alteracao de documentacao em `docs/canonicos/UX_BLUEPRINT.md`.
  - Redeploy da aplicacao no Coolify acionado para recurso `proj-financa-v1` (ambiente `production`).
...
- **Act:**
  - Registro de rastreabilidade atualizado no `docs/logs/PDCA_LOG.md`.
...
- **Do:**
  - Docs: atualizados `docs/canonicos/UX_BLUEPRINT.md`, `docs/canonicos/API_CONTRACT.md` e `docs/canonicos/MODELO_DADOS.md`.
...
- **Do:**
  ...
  - MCP do Coolify indisponivel nesta sessao; fallback operacional via push no `main` + validacao externa por HTTP.
- **Check:**
...
- **Act:**
  - Registro de deploy e validacao consolidado em `docs/logs/TEST_LOG.md`.
...
- **Do:**
  - Atualizado `docs/canonicos/RULES.md` com regra obrigatoria de seguranca de dados em deploy.
  - Atualizado `docs/canonicos/RUNBOOK.md` com guardrail pre-deploy e lista de comandos proibidos em producao.
  - Atualizado `docs/USAGE.md` removendo orientacao de escrita em banco de producao e adicionando lista de comandos proibidos.
...
- **Plan:** Corrigir falha de regra no CRUD de recorrencias onde auto-geracao ocorria no create, mas nao no update.
- **Do:**
...
- **Plan:** Criar skill reutilizavel para validar CRUDs do `proj-financa-v1` via navegador real, comparando UI e rede com `docs/canonicos/UX_BLUEPRINT.md` e `docs/canonicos/API_CONTRACT.md`.
- **Do:**
...
- **Act:** Proximo uso do skill deve gerar achados reproduziveis sobre UI/contrato sem precisar reconstruir matriz manualmente.

## 2026-03-13 — Skill local de design frontend (Claude Code `frontend-design`) (DEV-151)
- **Plan:** Criar skill local no repo inspirada no plugin `plugins/frontend-design` do Claude Code, para padronizar pedidos de UI com melhor qualidade de design.
- **Do:**
  - Referencia consumida via Context7 e leitura direta do GitHub (README + SKILL do plugin).
  ...
  - Criado runbook local em `.codex/napkin.md` para registrar padrao de skills locais.
...
- **Plan:** Corrigir inconsistência de contexto entre telas ao unificar competência mensal (YYYY-MM) em fonte única e expor seletor global na sidebar, sem impactar Panorama Anual.
- **Do:**
  - Docs: atualizado `docs/canonicos/UX_BLUEPRINT.md` para refletir seletor global de competência na sidebar e remoção de seletores locais do topo (Dashboard e Recorrências).
